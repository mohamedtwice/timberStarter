 ...
 */
function timberStarter_content_images_lightbox_rel( $content ) {
	global $post;
...
add_filter( 'the_content', 'timberStarter_content_images_lightbox_rel', 99 );
 ...
 */
function timberStarter_img_unautop( $pee ) {
	$pee = preg_replace( '/<p>\\s*?(<a .*?><img.*?><\\/a>|<img.*?>)?\\s*<\\/p>/s', '<figure>$1</figure>', $pee );

	return $pee;
}
add_filter( 'the_content', 'timberStarter_img_unautop', 30 );
 ...
 */
function timberStarter_cleaner_caption( $output, $attr, $content ) {
...
add_filter( 'img_caption_shortcode', 'timberStarter_cleaner_caption', 10, 3 );

/**
 * Clean the output of attributes of images in editor. Courtesy of SitePoint. http://www.sitepoint.com/wordpress-change-img-tag-html/
 */
function timberStarter_image_tag_class( $class, $id, $align, $size ) {
	$align = 'align' . esc_attr( $align );

	return $align;
}
add_filter( 'get_image_tag_class', 'timberStarter_image_tag_class', 0, 4 );

/**
 * Remove width and height in editor, for a better responsive world.
 */
function timberStarter_image_editor( $html, $id, $alt, $title ) {
...
add_filter( 'get_image_tag', 'timberStarter_image_editor', 0, 4 );
